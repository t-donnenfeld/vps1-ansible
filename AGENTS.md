...
5. Validate Authelia config with `docker run authelia config validate`
6. Render Grafana datasource provisioning (Loki + Prometheus)
7. Render Grafana dashboard provisioning config + copy Syncthing, Docker Containers, and Traefik dashboards
8. Fix volume permissions for UIDs 1000 (Vikunja, Navidrome, Syncthing, OpenClaw) and 10001 (Loki)
9. Install systemd unit + start stack
10. Create Navidrome user `theo` in SQLite DB via `docker compose exec navidrome sqlite3 ... INSERT OR IGNORE INTO user` (required for external auth via Authelia — fresh DB has no users and auto-creation doesn't work)
